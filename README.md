...
ChatHygiene is a self-hosted Telegram Business DM verification and anti-spam
service written in Rust. It watches private messages before the account owner
has replied, challenges new senders in the same conversation, and can remove
high-confidence spam using the permissions of a connected Business bot.

ChatHygiene is a post-delivery cleanup service, not a transparent messaging
gateway. Telegram first delivers a message to the Business account and then
sends the corresponding update to the bot. A notification or chat row may
therefore appear before ChatHygiene processes or deletes the message.
